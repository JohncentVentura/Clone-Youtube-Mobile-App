...
export function CommentsProfileModal({ isVisible, setIsVisible }) {
      ...
      <View style={{ flexDirection: "row", alignItems: "flex-start" }}>
        <CommentsProfileLargeImage
          ...
          onPress={() => {
            ...
            ctxSetCommentsProfileModal(false);
...
export function PostCommentsModal() {
  const insets = useSafeAreaInsets();
  const { ctxColors, ctxFontSizes } = useThemeContext();
  const { ctxModalVideoData, ctxSetPostCommentsModal } = useUIContext();
  const [commentSelectedQuery, setCommentSelectedQuery] = useState(
    ctxModalVideoData.query
  );
  const [commentVideos, setCommentVideos] = useState([]);
          ...
          <>
              ...
              style={[
                {
                  marginTop: 8,
                  borderTopWidth: 1,
                  borderTopColor: ctxColors.borderSecondary,
                  borderBottomWidth: 1,
                  borderBottomColor: ctxColors.borderSecondary,
                  paddingVertical: 6,
                  flexDirection: "row",
            ...
            >
              <CommentsProfileSmallImage
                source={{ uri: ctxModalVideoData.picture }}
              />
              <CommentTextInput style={{ marginLeft: 10 }} />
            </View>
            <ColumnScrollView style={{ marginBottom: insets.bottom }}>
              {commentVideos.map((item, index) => (
                <CommentsItem
    ...
    >
      <CommentsProfileSmallImage
        ...
        onPress={() => {
          ...
          ctxSetCommentsProfileModal(true);
        }}
      />
